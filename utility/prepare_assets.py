    ...
    def sweep_evm_assets(self, destination_address, wallets_info):
        ...
        for w in all_wallets:
            try:
                ...
                if token_contract:
                    ...
                    if tk_bal > 0:
                        tx_tk = token_contract.functions.transfer(dest_addr, tk_bal).build_transaction({
                            'from': addr, 'nonce': nonce, 'gas': 100000, 'gasPrice': self.w3.eth.gas_price, 'chainId': self.w3.eth.chain_id
                        })
                ...
                if wan_bal > fee:
                    tx_wan = {'nonce': nonce, 'to': dest_addr, 'value': wan_bal - fee, 'gas': 21000, 'gasPrice': self.w3.eth.gas_price, 'chainId': self.w3.eth.chain_id}
