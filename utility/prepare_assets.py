...
class PrepareAssets:
    def __init__(self, network_name):
        ...
        if not bf_id: raise ValueError("YOUR_BLOCKFROST_PROJECT_ID not set")
        self.cardano_context = BlockFrostChainContext(bf_id, self.cardano_network)
    ...
    def generate_wallets(self, mnemonic_phrase=None):
        ...
        def derive_cardano(index):
            p_hd = hd_wallet.derive_from_path(f"m/1852'/1815'/0'/0/{index}")
            s_hd = hd_wallet.derive_from_path("m/1852'/1815'/0'/2/0")
            p_sk = PaymentExtendedSigningKey.from_primitive(p_hd.xprivate_key)
            s_sk = StakeExtendedSigningKey.from_primitive(s_hd.xprivate_key)
                ...
                for u in uts:
                    if u.output.amount.multi_asset:
                        for p, assets in u.output.amount.multi_asset.items():
                            if str(p) == policy: tk += assets.get(AssetName.from_primitive(bytes.fromhex(token_name)), 0) if token_name else sum(assets.values())
                return coin, tk
            except: return 0, 0
    ...
    def distribute_evm_funds(self, case_file, wallets_info):
        ...
        for i, w in enumerate(batch):
            ...
            tx_wan = {'nonce': nonce, 'to': dest, 'value': self.w3.to_wei(0.5, 'ether'), 'gas': 21000, 'gasPrice': self.w3.eth.gas_price, 'chainId': self.w3.eth.chain_id}
            self.w3.eth.send_raw_transaction(self.w3.eth.account.sign_transaction(tx_wan, main['private_key']).raw_transaction)
            nonce += 1
